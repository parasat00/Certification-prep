# LPIC 101: A simple personal cheatsheet
...
**locate** command can be used to quickly find a file matching a given pattern on a Linux system. 
But unlike **find**, **locate** will not search the filesystem for the pattern. 
Instead, it looks it up on a database built by running the **updatedb** command. This gives you very quick results, but they may be imprecise depending on when the database was last updated.

The behavior of **updatedb** can be controlled by the file **/etc/updatedb.conf**
+ PRUNEFS=            Any filesystem types indicated after this parameter will not be scanned by updatedb. Names are case insensitive and should be separated by space
+ PRUNENAMES=         List of directory names that should not be scanned by updatedb. Names should be separated by space
+ PRUNEPATHS=         List of path names, separated by spaces, that should be ignored by updatedb. The path names must be specified in the same way they would be shown by updatedb. F.e: /var/spool/media
...
```bash
locate [pattern]              # Searches for file. F.e:
      locate jpg              # Displays all files which contain jpg (*jpg*) in the name of file

# By default the pattern is case sensitive.
locate -i [pattern]           # Case insensitive search

locate [pattern] [pattern]    # Searches for files containing either of the pattern in their name
locate -A [pattern] [pattern] # Searches for files containing both of the patterns in their name, F.e:
      locate -A .jpg .zip     # Will return files with such names: /home/me/a.jpg.zip

locate -c [pattern]           # Displays count of files that match the pattern

# One problem with locate is that it only shows entries present in the database generated by updatedb (located in /var/lib/mlocate.db).
# If the database is outdated, the output could show files that have been deleted since the last time it was updated.
locate -e [pattern]           # Displays all existent files that match pattern (-e prints only if file exists)
...
```bash
which [command]               # prints full path to executable
which -a [command]            # prints all path names that match the executable

type [command]                # prints path to executable
type -a [command]             # prints all path names that match the executable
type -t [command]             # prints type of the executable. Can be alias, keyword, function, builtin or file

whereis [command]             # prints paths to binary, man pages and source code(if available)
whereis -b [command]          # prints path to binary/executable
whereis -m [command]          # prints paths to man pages
whereis -s [command]          # prints path to source code (if available)
```
